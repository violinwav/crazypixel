...
// than free text: the room is four to six friends who already know each other, and a text
// field would need moderation, length limits, and an on-screen keyboard covering the hand
// panel on exactly the device (a phone) this is most played on.
...
export interface Emote {
   ...
   * covers only 1 of these 10 (no kana, CJK, Mongolian or Samaritan glyphs), and per-glyph
   * browser fallback would draw half of one kaomoji in each face at mismatched widths and
   * baselines. See .emote__glyph in theme.css.
   */
  text: string;
  /**
   * Spoken form. A kaomoji reads as a stream of character names to a screen reader ("white
   * circle with dot right, low line, ..."), so this is what gets announced and what labels
   * the button in the picker.
   */
  label: string;
}
